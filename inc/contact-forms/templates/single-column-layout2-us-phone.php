...
$html.= '<form id="'.$contactform[name].'_contactforms" class="awp_contact_'.$contactform[name].'" name="'.$contactform[name].'_contactforms" action="'.$_SERVER['REQUEST_URI'].'" method="post">';
...
foreach($formfields as $field)
	{
		...
		switch($fieldtype)
		{
			...
			break;
			case "textarea":
				$html .=  '<textarea  name="'.$fieldid.'" id="'.$fieldid.'_id" value=""  class="absp_contact_textarea'.$validateclass.'" size="50"></textarea>';
			break;
			case "select":
                              if($fieldid == 'country'){
