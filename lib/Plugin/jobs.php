...
if(strlen(trim($plugin_data['Apptivo Template Name'])) != 0 )
{
	...
	function createformfield_array($fieldid,$showtext,$required,$type,$validation,$options,$displayorder){
		...
		$hrjobsformfield= array(
	            'fieldid'=>$fieldid,
                'showtext' => preg_replace('/[^\w\+\s]/', '', $showtext),
	            'required' => $required,
				'type' => $type,
	...
	if($_POST['awp_jobs_add'] && ($_POST['nogdog'] == $_SESSION['apptvo_single_jobs']))  //Create Jobs
	    ...
	    ?><div class="icon32" style="margin-top:10px;background: url('<?php echo awp_image('jobs_icon'); ?>') " ><br></div> 
            ...
            <a class="nav-tab" href="/wp-admin/admin.php?page=awp_jobs&keys=jobsearch"><?php _e('Job Search','apptvo-businesssite'); ?></a>
            </h2>
	...
	?>
	   <p>
	   <img id="elementToResize"  src="<?php echo awp_flow_diagram('jobs');?>" alt="Jobs" title="Jobs"  />
	   </p>
	   <script type="text/javascript" language="javascript" >
	    ...
	    elem.style.width = wid+'px'; 
            ...
            <span style="margin:10px;">For Complete instructions, see the - <a href="<?php echo awp_developerguide('jobs');?>" target="_blank">Developer's Guide.</a></span>
	...
	if($_GET['action'] == 'edit' && is_numeric($_GET['id']))
		 ...
		 <h2><?php _e('Update Jobs','apptivo-businesssite'); ?></h2>
		  ...
		  <?php if(strlen($sucMsg) != 0 ) { ?><div id="message" style="width:80%;" class="updated below-h2"> <p><?php echo $sucMsg; ?></p></div><?php } ?>
		 </div>
           	<form name="awp_updatejobs" action="/wp-admin/admin.php?page=awp_jobs&keys=jobcreation" method="post" onsubmit="return validateupdatejobs(this)" >
           	<input type="hidden" id="job_ID" value="job_ID" value="<?php echo $_GET['id']; ?>" />
            <table width="700" cellspacing="0" cellpadding="0" class="form-table">
                                     <tbody><tr>
                                        ...
                                        <td><input type="text" size="43" id="jobs_title" name="jobs_title" value="<?php if(isset($_POST['jobs_title'])) { echo $_POST['jobs_title']; } else {  echo $selectedJobs->jobTitle; } ?>" >
                                        <input type="hidden" id="jobs_id" name="jobs_id" value="<?php echo $selectedJobs->jobId; ?>" />  </td>
                                    </tr>
                                    <tr>
             ...
             the_editor($updated_value,'editcontent','',FALSE);  ?>
                                    ...
                                    <tr>
                                        ...
                                        <td> 
                                        <select id="jobs_industry" name="jobs_industry">
                                        <?php foreach($allIndustries as $industries)
                                        {
                                       ?> <option <?php selected($industries->industryId, $selectedJobs->industryId); ?> value="<?php echo $industries->industryId;?>"><?php echo $industries->industryName;?></option>
                                        <?php 
                                        }?>
                                        </select>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                     <select id="jobs_type" name="jobs_type">
                                     <?php 
            						 foreach($jobTypeLists  as $key => $value)
                                     {
                                     ?><option <?php selected($key, $selectedJobs->jobTypeName); ?> value="<?php echo $key; ?>" <?php selected($key, $selectedJobs->jobTypeCode); ?> >
                                     ...
                                     {
                                     ?><option <?php selected($key, $selectedJobs->jobStatusName); ?> value="<?php echo $key; ?>" <?php selected($key, $selectedJobs->jobStatusName); ?> >
                                     <?php echo $value; ?></option><?php 
                                     }
                                     ?>												
                                        </select>
                                       ...
                                       <tr>
                                        ...
                                        <td>                                        
                                        <input <?php checked('Y',$selectedJobs->isFeatured); ?> type="checkbox" id="jobs_featured" name="jobs_featured" />                                        
                                        </td>
                                    </tr>
                                    ...
                                    <tr>
                                        <td></td>
                                        <td><input type="submit" class="button-primary" name="awp_updatejobs" value="<?php _e('Update Jobs','apptivo-businesssite');?>"></td>
                                    </tr>
        ...
        <input type="hidden" name="nogdog" value="<?php echo $nogdog;?>" >
            <table width="700" cellspacing="0" cellpadding="0" class="form-table">
                                     <tbody><tr>
                                        ...
                                        <input type="text" size="43" value="" id="jobs_title" name="jobs_title"></td>
                                    </tr>
                                    <tr>
                                        <td valign="top"><?php _e('Description','apptivo-businesssite');?>&nbsp;<span style="color:#f00;">*</span></td>
                                        <td>
                                    ...
                                    <tr>
                                     ...
                                     <?php echo $value; ?></option><?php 
                                       ...
                                       <tr>
                                        <td><?php _e('Is featured','apptivo-businesssite');?></td>
                                        <td>                                        
                                        <input type="checkbox" id="jobs_featured" name="jobs_featured" />                                        
                                        </td>
                                    </tr>
                                    ...
                                    <tr>
                                        <td></td>
                                        <td><input <?php if(!$this->_plugin_activated) { echo 'disabled="disabled"'; }?> type="submit" class="button-primary" name="awp_jobs_add" value="<?php _e('Create Jobs','apptivo-businesssite');?>"></td>
                                    </tr>
...
function validatecreatejobs()
 {   
	...
	$numberofjobs = count($JobSearchResults);
   	...
   	?>
		...
		?>	
		<form name="awp_jos_deleteform" method="post" action="/wp-admin/admin.php?page=awp_jobs">
		<input type="hidden" name="job_delete_form" id="job_delete_form"  />	
       ...
       <tbody id="the-list">
                                   ...
                                   if(!empty($JobSearchResults)){
                                    ...
                                    ?><tr <?php echo $style; ?> >
                                            ...
                                            <td><div><p>
                                            <?php if (strlen(strip_tags(html_entity_decode($jobs->jobDescription))) < 30)
		                                            {
                                    ...
                                    <?php  }
                                       ...
                                       ?><tr class="no-items"><td class="colspanchange" colspan="5"><?php _e('No jobs found','apptivo-businesssite');?></td></tr><?php
                                   }
                                    ?>
               </tbody>
       </table>
 ...
 jQuery(document).ready(function(){
		jQuery("#jobs_shortcode").focus(function(){
	    	this.select();
	    });	
...
<?php 
		...
		<table class="form-table">
			<tbody>
				<tr valign="top">
					...
					<td valign="top">
                  	<input style="width:350px;" name="awp_jobsform_select_form" id="awp_jobsform_select_form" type="text" readonly="readonly" value=<?php echo $selectedhrjobsform; ?> />
					</td>					
				</tr>
				<tr valign="top">
					<th valign="top"><label for="awp_customform_shortcode"><?php _e("Form Shortcode", 'apptivo-businesssite' ); ?>:</label>
					<br><span class="description"><?php _e("Copy and Paste this short code in your page to display the job applicant form.", 'apptivo-businesssite' ); ?></span>
					</th>
					<td valign="top"><span id="awp_customform_shortcode" name="awp_customform_shortcode">
					<input style="width:350px;" type="text" readonly="readonly" id="job_applicantform_shortcode" name="job_applicantform_shortcode"  value='[apptivo_job_applicantform name="<?php echo $selectedhrjobsform?>"]' />
					</span>
					
					<span style="margin:10px;">*Developers Guide - <a href="<?php echo awp_developerguide('job-applicant-shortcode');?>" target="_blank">Job Applicant Form Shortcodes.</a></span>
					
					</td>
				</tr>
			</tbody>
		</table>
		<?php } else { echo '<span style="color: rgb(255, 0, 0);line-height:24px;"> Save the the below settings to get the Shortcode for job applicant form. </span>'; } ?>
	...
	<form name="awp_jobs_settings_form" method="post" action="">
		<table class="form-table">
			<tbody>
					...
					<br><span class="description" valign="top"></span>
                     ...
                     <td valign="top">
                       <select id="awp_jobapplicant_page" name="awp_jobapplicant_page" >
                      
							 <?php
							  $pages = get_pages(); 
							  foreach ($pages as $pagg) {
							  	?>
							  	<option value="<?php echo $pagg->ID; ?>"  <?php selected($pagg->ID, $formproperties[jobapplicant_page]); ?> >
				...
				<tr valign="top">
					...
					<td valign="top">
					<input type="hidden" id="awp_jobsform_name" name="awp_jobsform_name" value="<?php echo $selectedhrjobsform;?>" > 
					<select name="awp_jobsform_templatetype" id="awp_jobsform_templatetype" onchange="changeTemplate();" > 
							<option value="awp_plugin_template" <?php selected($formproperties[tmpltype],'awp_plugin_template'); ?> > <?php _e("Plugin Templates", 'apptivo-businesssite' ); ?></option>
							<?php if(!empty($themetemplates)) : ?>
							<option value="theme_template" <?php selected($formproperties[tmpltype],'theme_template'); ?> > <?php _e("Templates from Current Theme", 'apptivo-businesssite' ); ?></option>
							<?php endif; ?>
					</select>
					<span style="margin:10px;">*Developers Guide - <a href="<?php echo awp_developerguide('job-applicant-template');?>" target="_blank">Job Applicant Form Templates.</a></span>
					</td>
				</tr>
				<tr valign="top">
					<th valign="top"><label for="awp_jobsform_templatelayout"><?php _e("Template Layout", 'apptivo-businesssite' ); ?>:</label>
					<br><span class="description"><?php _e("Selecting Theme template which doesnt support jobs form structure will wont show the jobs form in webpage.", 'apptivo-businesssite' ); ?></span>
					</th>
					<td valign="top">
					<select name="awp_jobsform_plugintemplatelayout" id="awp_jobsform_plugintemplatelayout" <?php if($formproperties['tmpltype'] == 'theme_template' ) echo 'style="display: none;"'; ?> >
					...
					<select name="awp_jobsform_themetemplatelayout" id="awp_jobsform_themetemplatelayout" <?php if($formproperties['tmpltype'] != 'theme_template' ) echo 'style="display: none;"'; ?> >
				...
				<tr valign="top">
					...
					<br><span class="description">This message will shown in your website page, once jobs form submitted.</span>
					</th>
					<td valign="top">
				...
				<tr valign="top">
					...
					<td valign="top">
					<textarea name="awp_jobsform_customcss" style="width:350px;" id="awp_jobsform_customcss" size="100" cols="40" rows="10"><?php echo $formproperties[css];?></textarea>
					<span style="margin:10px;">*Developers Guide - <a href="<?php echo awp_developerguide('job-applicant-customcss');?>" target="_blank">Job Applicant Form CSS.</a></span>
					</td>
				</tr>
                                <tr valign="top">
					<th><label id="awp_jobsform_submit_type" for="awp_jobsform_submit_type"><?php _e("Submit Button Type", 'apptivo-businesssite' ); ?>:</label>
					<br><span valign="top" class="description"></span>
					</th>

                                        <td valign="top">
                                            <input type="radio" value="submit" name="awp_jobsform_submit_type" <?php checked('submit',$formproperties[submit_button_type]); ?> checked="checked" /> Button
                                            <input type="radio" value="image" name="awp_jobsform_submit_type"<?php checked('image',$formproperties[submit_button_type]); ?>/> Image
					</td>
				</tr>
                                 <tr valign="top">
		...
		<div style="amrgin:10px;">
		<span class="description"><?php _e("Select and configure list of fields from below table to show in your jobs form.", 'apptivo-businesssite' ); ?></span>
		<span style="margin:10px;">*Developers Guide - <a href="<?php echo awp_developerguide('job-applicant-basicconfig');?>" target="_blank">Basic Job Applicant Form Config.</a></span>
		</div>
		<br>
		<table width="900" cellspacing="0" cellpadding="0" id="hrjobs_form_fields" name="hrjobs_form_fields" style="border-collapse: collapse;">
			<tbody>
				...
				<tr>
					...
					<input
					<?php  if($enabled) { ?> checked="checked" <?php } if($fieldData['fieldid']=='lastname' || $fieldData['fieldid']=='email' || $fieldData['fieldid']=='firstname' || $fieldData['fieldid']=='country'){?> disabled="disabled" <?php } ?> type="checkbox"  id="<?php echo $fieldData['fieldid']?>_show" name="<?php echo $fieldData['fieldid']?>_show" size="30"
					...
					<?php
						...
						checked="checked" <?php }?>type="checkbox"
                                         <?php if($fieldData['fieldid']=='lastname' || $fieldData['fieldid']=='email'|| $fieldData['fieldid']=='firstname' || $fieldData['fieldid']=='country' ){?> disabled="disabled" <?php } ?>
				   ...
				   if($fieldData['fieldid'] == 'industry')
					...
					<select id="<?php echo $fieldData['fieldid']?>_options" name="<?php echo $fieldData['fieldid']?>_options[]" size="4" multiple="multiple" style="height:75px;">
					  ...
					  foreach($allIndustries as $industries)
                                        {
                                       ?>
                                       <option <?php if( in_array($industries->industryId.'::'.$industries->industryName, $fieldData['options'])) { echo 'selected="selected"';}  ?> value="<?php echo $industries->industryId.'::'.$industries->industryName;?>"><?php echo $industries->industryName;?></option>
                                        <?php 
                                        }?>
					...
					if(($fieldData['type']=="select")||($fieldData['type']=="radio")||($fieldData['type']=="checkbox")) { 
					  ?>
					<textarea style="width:190px;" <?php if(!$enabled){ ?> disabled="disabled" <?php } ?> id="<?php echo $fieldData['fieldid']?>_options" name="<?php echo $fieldData['fieldid']?>_options" ><?php echo $fieldData['options']; ?></textarea>
					<?php }else {?>
					<textarea disabled="disabled" style="display:none;width:190px;" id="<?php echo $fieldData['fieldid']?>_options" name="<?php echo $fieldData['fieldid']?>_options" ></textarea>
					<?php }  ?>
		...
		<p class="submit">
			<input <?php if(!$this->_plugin_activated) { echo 'disabled="disabled"'; } ?> type="submit" name="awp_jobsform_settings" id="awp_jobsform_settings" class="button-primary" value="<?php esc_attr_e('Save Configuration') ?>" />
		</p>
	</form>
...
jQuery(document).ready(function(){
	...
	{   
		jQuery('#hrjobs_form_fields tr:last').after('<tr><td style="border: 1px solid rgb(204, 204, 204); padding-left: 10px; width: 150px;">Custom Field '+ counter + '</td><td align="center" style="border: 1px solid rgb(204, 204, 204);">' + 
				'<input type="checkbox" id="customfield'+ counter + '_show" name="customfield'+ counter + '_show" size="30" onclick="hrjobsform_enablefield(\'customfield'+counter+'\')">' +
				...
				'</td>' +				
				'<td align="center" style="border: 1px solid rgb(204, 204, 204);">' +
				'<select name="customfield'+ counter + '_type" id="customfield'+ counter + '_type" onchange="hrjobsform_showoptionstextarea(\'customfield'+counter+'\');" disabled="">' +
...
function hrjobsform_showoptionstextarea(fieldid){
	...
	{ 
		...
		{ ?>
		<p>Copy and Paste this short code in Job Description Page  : <input type="text" readonly="readonly" id="job_description_shortcode" name="job_description_shortcode" value ="[apptivo_job_description]" /> </p>
		<?php } else { echo '<p style="color:#f00;">Save the the below settings to get the Shortcode for job description.</p>'; } ?>
		<form action="" method="post" name="awp_joblists_settings_form">
       <table class="form-table">
			<tbody>               
               
				 <tr valign="top">
					<th><label id="awp_jobsearchform_submit_type" for="awp_joblist_submit_type"><?php _e("Submit Button Type", 'apptivo-businesssite' ); ?>:</label>
					<br><span valign="top" class="description"></span>
					</th>

                   <td valign="top">
                    <input type="radio" value="submit" name="awp_joblist_submit_type" <?php checked('submit',$jobs_settings[submit_type]); ?> checked="checked" /> Button
                    <input type="radio" value="image" name="awp_joblist_submit_type"<?php checked('image',$jobs_settings[submit_type]); ?>/> Image
					</td>
				</tr>
                <tr valign="top">
				...
				<tr valign="top">
					...
					<td valign="top">
					<select id="awp_joblist_descriptionpage" name="awp_joblist_descriptionpage">					
							 <?php
							  $pages = get_pages(); 
							  foreach ($pages as $pagg) {
							  	?>
							  	<option value="<?php echo $pagg->ID; ?>"  <?php selected($pagg->ID, $jobs_settings[description_page]); ?> >
														<?php echo $pagg->post_title; ?>
								</option>
							  	<?php 
							  }
				...
				<td valign="top">
				 <?php $awp_jobdesc_themetemplates = get_awpTemplates(TEMPLATEPATH.'/jobs/jobdescription','Plugin');     ?>
				
				<select name="awp_jobdesc_templatetype" id="awp_jobdesc_templatetype" >
                	<option value="awp_plugin_template" <?php selected($jobs_settings['jobdescription_template_type'],'awp_plugin_template'); ?> ><?php _e('Plugin Templates','apptivo-businesssite'); ?></option>
                    <?php if(!empty($awp_jobdesc_themetemplates)) : ?>
                    <option value="theme_template" <?php selected($jobs_settings['jobdescription_template_type'],'theme_template'); ?> >Templates from Current Theme</option>
                    <?php endif; ?>
                </select>
				...
				?>
				<select name="awp_jobdesc_template" id="awp_jobdesc_template" <?php if($jobs_settings['jobdescription_template_type'] == 'theme_template' ) echo 'style="display:none;"'; ?> >
						<?php foreach (array_keys( $plugintemplates ) as $template ) { ?>
							<option value="<?php echo $plugintemplates[$template]?>" <?php selected($plugintemplates[$template], $jobs_settings[desc_template_name]); ?> 	>
							<?php echo $template?>
							</option>
							<?php }?>
				</select>
				
				<select name="awp_jobdesc_theme_template" id="awp_jobdesc_theme_template" <?php if($jobs_settings['jobdescription_template_type'] != 'theme_template' ) echo 'style="display:none;"'; ?> >
                                              <?php foreach (array_keys($awp_jobdesc_themetemplates) as $template) : ?>
                                   			  <option value="<?php echo $awp_jobdesc_themetemplates[$template] ?>" <?php selected($awp_jobdesc_themetemplates[$template],$jobs_settings['desc_template_name']); ?>>  <?php echo $template ?> </option>                 
                            				  <?php endforeach; ?>
                </select>
				...
				<td valign="top">
				 <?php $awp_joblists_themetemplates = get_awpTemplates(TEMPLATEPATH.'/jobs/joblists','Plugin');  ?>
				
				<select name="awp_joblists_templatetype" id="awp_joblists_templatetype" >
                	<option value="awp_plugin_template" <?php selected($jobs_settings['joblist_template_type'],'awp_plugin_template'); ?> ><?php _e('Plugin Templates','apptivo-businesssite'); ?></option>
                    <?php if(!empty($awp_joblists_themetemplates)) : ?>
                    <option value="theme_template" <?php selected($jobs_settings['joblist_template_type'],'theme_template'); ?> >Templates from Current Theme</option>
                    <?php endif; ?>
                </select>
				...
				<td valign="top">
				<?php $plugintemplates =get_awpTemplates(AWP_JOBLISTS_TEMPLATEPATH,'plugin');  ?>
				<select name="awp_joblist_template" id="awp_joblist_template" <?php if($jobs_settings['joblist_template_type'] == 'theme_template'  ) echo 'style="display:none;"'; ?> >
						<?php foreach (array_keys( $plugintemplates ) as $template ) : ?>
						<option value="<?php echo $plugintemplates[$template]?>" <?php selected($plugintemplates[$template], $jobs_settings['list_template_name']); ?> >
						<?php echo $template?>
						</option>
						<?php endforeach; ?>
				</select>
				
				<select name="awp_joblists_theme_template" id="awp_joblists_theme_template" <?php if($jobs_settings['joblist_template_type'] != 'theme_template' ) echo 'style="display:none;"'; ?> >
                       <?php foreach (array_keys($awp_joblists_themetemplates) as $template) : ?>
                       <option value="<?php echo $awp_joblists_themetemplates[$template] ?>" <?php selected($awp_joblists_themetemplates[$template], $jobs_settings['list_template_name']); ?>  >  <?php echo $template ?> </option>                 
                       <?php endforeach; ?>
                </select>
		...
		<p class="submit">
			<input <?php if(!$this->_plugin_activated) { echo 'disabled="disabled"'; } ?> type="submit" value="<?php _e('Save Configuration','apptivo-businesssite'); ?>" class="button-primary" id="awp_joblists_settings" name="awp_joblists_settings">
		</p>
	</form> 
	<script type="text/javascript" language="javascript" >
	...
	<div class="icon32" style="margin-top:10px;background: url('<?php echo awp_image('jobs_icon'); ?>') " ><br></div> 
            ...
            <a class="nav-tab nav-tab-active" href="/wp-admin/admin.php?page=awp_jobs&keys=jobsearch"><?php _e('Job Search','apptivo-businesssite'); ?></a>
            </h2>
         <?php    
            if(!$this->_plugin_activated) :
	   ...
	   <?php if(!empty($formproperties)) {?>
		<table class="form-table">
			<tbody>
				<tr valign="top">
					...
					<input style="width:360px;" type="text" readonly="readonly" id="awp_jobsearchform_select_form" name="awp_jobsearchform_select_form" value="<?php echo $selectedhrjobsform; ?>" />
				...
				<tr valign="top">
					...
					<br><span class="description"><?php _e('Copy and Paste this short code in your page to display the job search form.', 'apptivo-businesssite' ); ?></span>
					</th>
					<td valign="top"><span id="awp_customform_shortcode" name="awp_customform_shortcode">
					<input style="width:360px;" type="text" readonly="readonly" id="job_searchform_shortcode" name="job_searchform_shortcode" value='[apptivo_job_searchform name="<?php echo $selectedhrjobsform?>"]' />
					</span>
					<span style="margin:10px;">*Developers Guide - <a href="<?php echo awp_developerguide('job-searchform-shortcode');?>" target="_blank">Job Search Form Shortcodes.</a></span>
		...
		<?php } else { 
	  echo '<span style="color: rgb(255, 0, 0);line-height:24px;"> Save the the below settings to get the Shortcode for job search form. </span>';	
	  }?>
	...
	<form name="awp_jobsearch_settings_form" method="post" action="">
			...
			<tbody>
				...
				<tr valign="top">
					...
					<td valign="top"><input type="hidden" id="awp_jobsearchform_name"
						name="awp_jobsearchform_name" value="<?php echo $selectedhrjobsform;?>"> 
					<select name="awp_jobsearchform_templatetype" id="awp_jobsearchform_templatetype" onchange="change_searchform_Template();">
					...
					<span style="margin:10px;">*Developers Guide - <a href="<?php echo awp_developerguide('job-searchform-template');?>" target="_blank">Job Search Form Templates.</a></span>
					</td>
				</tr>
				<tr valign="top">
					<th valign="top"><label for="awp_jobsearchform_templatelayout"><?php _e("Template Layout", 'apptivo-businesssite' ); ?>:</label>
					<br><span class="description"><?php _e("Selecting Theme template which doesnt support jobs search form structure will wont show the jobs form in webpage.", 'apptivo-businesssite' ); ?></span>
					</th>
					<td valign="top">
					<select name="awp_jobsearchform_plugintemplatelayout" id="awp_jobsearchform_plugintemplatelayout" <?php if($formproperties['tmpltype'] == 'theme_template' ) echo 'style="display: none;"'; ?> >
						<?php foreach (array_keys( $plugintemplates ) as $template ) { ?>
							<option value="<?php echo $plugintemplates[$template]?>" <?php selected($formproperties[layout],$plugintemplates[$template]); ?> >
							<?php echo $template; ?>
							</option>
					 ...
					 <select name="awp_jobsearchform_themetemplatelayout" id="awp_jobsearchform_themetemplatelayout" <?php if($formproperties['tmpltype'] != 'theme_template' ) echo 'style="display: none;"'; ?> >
						<?php foreach (array_keys( $themetemplates ) as $template ){ ?> 
							<option value="<?php echo $themetemplates[$template]?>" <?php selected($formproperties[layout],$themetemplates[$template]);?> >
							<?php echo $template; ?>
							</option>
				...
				<tr valign="top">
					...
					<td valign="top">
					<textarea style="width:360px;" name="awp_jobsearchform_customcss" id="awp_jobsearchform_customcss" size="100" cols="40" rows="10"><?php echo $formproperties[css];?></textarea>
					<span style="margin:10px;">*Developers Guide - <a href="<?php echo awp_developerguide('job-searchform-customcss');?>" target="_blank">Job Search Form CSS.</a></span>
					</td>
				</tr>
                <tr valign="top">
					<th><label id="awp_jobsearchform_submit_type" for="awp_jobsearchform_submit_type"><?php _e("Submit Button Type", 'apptivo-businesssite' ); ?>:</label>
					<br><span valign="top" class="description"></span>
					</th>

                   <td valign="top">
                    <input type="radio" value="submit" name="awp_jobsearchform_submit_type" <?php checked('submit',$formproperties[submit_button_type]); ?> checked="checked" /> Button
                    <input type="radio" value="image" name="awp_jobsearchform_submit_type"<?php checked('image',$formproperties[submit_button_type]); ?>/> Image
					</td>
				</tr>
                    <tr valign="top">
		...
		<span class="description"><?php _e('Select and configure list of fields from below table to show in your hrjobs form.','apptivo-businesssite');?></span>
		<span style="margin:10px;">*Developers Guide - <a href="<?php echo awp_developerguide('job-searchform-basicconfig');?>" target="_blank">Basic Job Search Form Config.</a></span>
		</div>
		...
		<table width="900" cellspacing="0" cellpadding="0"
			...
			<tbody>
				...
				<tr >
					...
					onclick="hrjobsform_enablefield('<?php echo $fieldData['fieldid']?>')"></td> 
				...
				{ ?>
					...
					{ 
	                   ...
	                   <select id="<?php echo $fieldData['fieldid']?>_options" name="<?php echo $fieldData['fieldid']?>_options[]" size="4" multiple="multiple" style="height:60px;width:180px;">
            						 ...
            						 foreach($jobTypeLists  as $key => $value)
                                     {
                                     ?><option <?php if( in_array($key, $fieldData['options'])) { echo 'selected="selected"';}  ?> value="<?php echo $key; ?>" ><?php echo $value; ?></option>
                                     <?php 
                                     } ?>												
                       </select>
                   <?php }           
		...
		<p class="submit">
			<input <?php if(!$this->_plugin_activated) { echo 'disabled="disabled"'; } ?> type="submit" name="awp_jobsearchform_settings" id="awp_jobsearchform_settings" class="button-primary" value="<?php esc_attr_e('Save Configuration') ?>" />
		</p>
	</form>
 ...
 * @param unknown_type $jobDescription
 * @param unknown_type $industryId
 * @param unknown_type $jobtype
 ...
 * @param int $maxCount
 * @param int $offset
 * @param String $getFeaturedJobsOnly
 * @param string $status
 * @return HrJobSearchResultsHelper
 */
function getAllHrjobs($maxCount=999,$offset=0,$getFeaturedJobsOnly='false',$status = null)
{  
	$sortBy = 0;
 ...
 * @param ArrayLists $job_types
 * @param int $maxcount
 * @return HrJobSearchResultsHelper
 */
function serchByJobs($keyword,$industry,$job_types,$maxcount=999)
{  
	$params = array ( 
                "arg0" => APPTIVO_SITE_KEY,
				"arg1" => APPTIVO_ACCESS_KEY,
