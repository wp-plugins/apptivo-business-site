...
class Plugin_Cache_File
{
    ...
    {
        $cache_dir = isset($config['cache_dir']) ? trim($config['cache_dir']) :WP_CONTENT_DIR.'/apptivo-cache';
        if(!@is_dir($cache_dir))
            if(@mkdir ($cache_dir))
     ...
     */
    function Plugin_Cache_File($config = array())
    {
    ...
    function set($key, &$var, $expire = 0)
    {
        ...
        $dir = dirname($path);
        if ((@is_dir($dir) || awp_mkdir($sub_dir, 0755, $this->_cache_dir))) {
            $fp = @fopen($path, 'wb');
